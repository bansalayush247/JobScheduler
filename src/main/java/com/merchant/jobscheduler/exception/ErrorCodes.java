package com.merchant.jobscheduler.exception;

public class ErrorCodes {

    public static final String USER_NOT_FOUND = "ERR001";
    public static final String INVALID_CREDENTIALS = "ERR002";
    public static final String EMAIL_ALREADY_EXISTS = "ERR003";
    public static final String ROLE_NOT_FOUND ="ERR004";
    public static final String ADMIN_ALREADY_EXISTS= "ERR005";
    public static final String ADMIN_ROLE_CHANGE_NOT_ALLOWED= "ERR006";
}